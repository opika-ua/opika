 ...
 *
 * Drawing every page up to `MAX_GALLERY_PAGE`'s own bound (~83 pages at
 * 24/page) would mean up to 83 × 44px targets — even three rows of them
 * on a 390px phone. Below `ALWAYS_EXPANDED_THRESHOLD`, every page is drawn
 * with no gaps (small counts read better fully spelled out than
 * abbreviated). At or above it, only page 1, the last page, and a
 * `current`-1..`current`+1 band are drawn, with an "ellipsis" marker for
