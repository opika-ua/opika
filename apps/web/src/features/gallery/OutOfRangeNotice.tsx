 ...
 *
 * `#F2F2F0` fill / radius 16 / padding `16 20` / two 15px lines / an
 * underlined text link (navigation, not an operation — a `<Link>`, never a
 * `<button>`) are the mock's own literal values. Renders server-side, no
 * client component needed — unlike `loading.tsx`/`error.tsx`, this is the
 * one E4 state that works with JS disabled (`docs/gallery-contract-
 * decisions.md`'s JS-only-paths note).
 */
export function OutOfRangeNotice({
  ...
  return (
    <div
      ...
      className="font-rg bg-rg-fill rounded-rg-button py-4 px-5 flex flex-col gap-1 mb-4"
    >
      ...
      <Link
        ...
        className="text-[15px] text-rg-ink underline underline-offset-2 mt-1 focus-visible:outline focus-visible:outline-[3px] focus-visible:outline-rg-registry focus-visible:outline-offset-[3px]"
      >
