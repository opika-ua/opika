...
| D-2 | Compact demo banner inside the deck's existing header row (replace, don't add — zero new height) plus the `firstRun.promise` swap and `shelterVerifiedYears` badge suppression. Ukrainian ships real text, pinned empty of `[COPY PENDING]` by `copy-status.test.ts`. **Depends on:** D-1 | **Done, 2026-09-06 — Ukrainian landed, nothing renders a marker anywhere.** Decided (Oleksii, Phase D decisions): the banner replaces only `filtersLabel` — the position count stays (demo mode is the whole testing period; a deck missing the count for weeks isn't the deck being tested); the progress bar degrades instead, hidden unconditionally whenever the demo notice shows, which is what buys the notice its width. Recorded as a Phase D deviation in `docs/design/README.md`'s deck-chrome section. Width budget was measured before the copy landed, bar hidden: tightest at 320px (~107px available, a 12-character label fits, 14 clips), ~147px at 360px, ~177px at 390px — «Демо» (4 characters) is nowhere near that ceiling. **Amendment to the original `og:description` decision:** it said omit entirely rather than render `[COPY PENDING]` — a fallback for having no honest string. With `uk.demo.promise` real, every route uses it as its description while the flag is true, `/prytulkam` included — that's the link Oleksii sends to shelters, and a weaker title-only fallback there works against this phase's own purpose. The detail page's not-a-judgement notice (recovered verbatim, «просто» included, from the deleted `firstRun.disclaimer` — not build-plan's own D-3 row below — see `docs/observations.md`) is also real and live now, closing the question of whether it should match the banner/preview's withhold-while-pending treatment: nothing is pending anywhere, so the question no longer applies. **Corrected provenance, 2026-09-06 (`opika-reviewer` STOP):** `uk.demo.promise` and `uk.demo.bannerNotice` were drafted by Claude from an English sense, not authored by Oleksii — he chose these two from three offered options on 2026-09-05; approved wording, not his own. The notice's «просто»-dropping edit was Claude's unapproved change and has been reverted; the shipped sentence is Oleksii's own pre-existing copy, reproduced verbatim. Full record in `docs/observations.md`'s O-3 section. Also fixed in the same pass: `/tvaryny/[animalId]`'s own `generateMetadata` did not inherit the root layout's swap (Next overrides, not merges, route metadata) and previewed a fictional animal's age/size with no demo disclosure at all on the one URL `/prytulkam` tells shelters to paste into Telegram — now wired the same way, asserted in `seo-flags.test.ts` for both flag states |
